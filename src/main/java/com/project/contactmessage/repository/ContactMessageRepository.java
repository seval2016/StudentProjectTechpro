...
public interface ContactMessageRepository extends JpaRepository<ContactMessage, Long> {
   ...
   /* @Query("SELECT c FROM ContactMessage c WHERE " +
            "(FUNCTION('HOUR' , c.dateTime) BETWEEN : ?1 AND : ?3) AND " +
            "(FUNCTION('HOUR' , c.dateTime) != :?1 OR FUNCTION('MINUTE' , c.dateTime) >= :?2) AND " +
            "(FUNCTION('HOUR' , c.dateTime) != :?3 OR FUNCTION('MINUTE' , c.dateTime) <= :?4)")
    List<ContactMessage> findMessagesBetweenTimes(int startHour,int startMinute,int endHour,int endMinute);
     ...
     // http://localhost:8080/contactMessages/searchBetweenTimes?startHour=09&startMinute=15&endHour=17&endMinute=30
		Gelen sorguda gelen parametreler --> 09:15 / 17:30
